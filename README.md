...
Current userscript version: **1.1.475**  
...
## What's New in v1.1.475

- Fixed Saved Reports so a completed calculation gets a second save attempt if the first backend write fails.
- If both save attempts fail, RWPH now shows the Saved Reports error instead of silently leaving the report unsaved.
- Added faction-level **Auto Delete** controls to the Cached Reports panel.
- Auto Delete can be turned on/off and set to remove reports after **1h, 3h, 6h, 12h, 24h, 48h, 3 days, or 7 days**.
- Expired reports are removed by the backend before they count toward the faction's 3 saved slots.
- Cached Reports refreshes when the panel is first opened and immediately after deleting a report. It does not poll continuously.
- Existing 3-slot behaviour remains: if all active slots are full, RWPH opens Cached Reports instead of starting another calculation.
...
- If all 3 slots are full, RWPH opens Cached Reports and does not start another calculation until you delete one report or Auto Delete frees a slot.
...
The previous automatic cache matching, mode-specific Use Cached Report buttons, and Delete Cache cooldown are no longer used.
...
## Current Version Summary — v1.1.475

RWPH v1.1.475 uses the faction-level **Cached Reports** panel with three saved-report slots per faction. Completed calculations save automatically, and RWPH retries the save once if the first database write fails. The panel refreshes only when opened and after deleting a report. Optional faction-level Auto Delete can remove reports after 1, 3, 6, 12, 24, 48, 72, or 168 hours. Normal calculations always run fresh and require an empty saved-report slot before starting.
